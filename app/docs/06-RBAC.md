...
|------|-------------|-------------|
| **Local User** | Has full access to all application features via the browser UI. No authentication required. Access is implicitly granted by being on the local machine. | End user operating the machine |
| **API Consumer** | Accesses the REST API and WebSocket directly (e.g., via script or test harness). Same permissions as Local User. | Developer, automated test |
...
| **Setup Administrator** | Runs `setup.bat` to create virtual environments and install dependencies. Requires write access to the application directory. | DevOps, initial installer |

### OS-Level Roles

| Role | OS Permissions Required |
|------|------------------------|
| **Local User** | Read access to `C:\saabdemo\app\src\app\templates\`; read access to `C:\saabdemo\app\static\` |
| **Server Operator** | Read/write access to `C:\saabdemo\app\.env`; read/write to `C:\saabdemo\app\generated-apps\`; execute permission on `.venv\Scripts\` |
| **Setup Administrator** | Write access to `C:\saabdemo\app\` and all subdirectories; execute permission for Python installer |

### Container Role

| Role | Description |
|------|-------------|
| **appuser** | Non-root container user created during image build. Has write access only to `/home/appuser/app/generated-apps/`. Cannot write to `/`, `/bin`, `/usr`, or other system directories. Follows CIS Benchmark Level 2 least-privilege principle. |
...
|----------|---------------|----------------|
| AI provider API key (in memory) | Secret | Process memory only — no external access; held in `state._provider` |
...
All endpoints are accessible from `127.0.0.1` without authentication in the current deployment model.

| Endpoint | Access Control | Notes |
|----------|---------------|-------|
| `GET /api/health` | Open | No credential required |
| `POST /api/config` | Open | API key validated against provider, then held in memory |
| `POST /api/plan` | Requires prior `/api/config` success | Returns HTTP 400 if provider not set |
| `POST /api/generate` | Requires prior `/api/config` success | Returns HTTP 400 if provider not set; 409 if generation in progress |
| `GET /api/status` | Open | Read-only state query |
| `POST /api/stop` | Open | Idempotent — safe to call when idle |
...
|-----------|-------|--------------|
| User | `appuser` (UID > 1000) | CIS Benchmark L2: no root container processes |
...
| Read-only paths | `/home/appuser/app/src/`, `/home/appuser/app/base-template/` | Application source is static |
| Exposed port | `8000` (TCP) | Orchestrator port only |
| Capabilities | Default dropped | `--cap-drop ALL` recommended at runtime |
| `--read-only` flag | Recommended with `--tmpfs /tmp` | Immutable root filesystem |
...
| Keys never logged | No `logger.*` calls reference `api_key`; logging formatters do not log request bodies | NIST AU-9 |
| Keys never returned in responses | No endpoint returns the stored key | OWASP A02 |
| Keys cleared on server restart | Module-level `_provider = None` is the initial state | NIST IA-5 |
| Key validation before storage | Probe call to provider made in `POST /api/config` before `set_provider()` | NIST IA-5 |
| Key transmission | HTTPS should be used in production; localhost HTTP acceptable for local-only | OWASP A02 |

---

## 10. RBAC Gaps and Roadmap

The following gaps exist in the current implementation and should be addressed before any network-accessible deployment:
...
| No session management | Concurrent users on the same machine share state | Add per-session provider storage and state isolation |
| WebSocket has no authentication | Any localhost client can receive log messages | Add token query parameter to WebSocket URL |
| `/docs` Swagger UI is publicly accessible | Exposes API schema to any localhost process | Disable `docs_url` in production or restrict to admin role |
| No rate limiting on API endpoints | A script could flood `/api/generate` | Add FastAPI rate limiting middleware (e.g., `slowapi`) |
| Generated app inherits OS user permissions | Generated code runs as the same user as the orchestrator | Run generated app in a separate OS account or container |
...
| Review OS account permissions on deployment host | Every 6 months | Security Officer / DevOps |
| Review RBAC gaps and roadmap | Every release | Security Officer |
...
*Document maintained at `C:\saabdemo\app\docs\06-RBAC.md`*  
